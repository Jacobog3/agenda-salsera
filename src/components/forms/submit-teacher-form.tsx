...
export function SubmitTeacherForm() {
  ...
  const idempotencyKey = useRef(createSubmissionIdempotencyKey("teacher"));
  ...
  async function handleParse() {
    ...
    setParseError("");
    try {
      const d = await analyzeSubmissionMaterial({ type: "teacher", text: sourceText, imageFile });
      ...
      setFields((prev) => ({
        ...
        classFormats: d.classFormats || prev.classFormats,
        teachingVenues: d.teachingVenues || prev.teachingVenues,
        scheduleText: d.scheduleText || prev.scheduleText,
        whatsapp: d.whatsapp || prev.whatsapp,
        instagram: d.instagram || prev.instagram,
        website: d.website || prev.website,
        bookingUrl: d.bookingUrl || prev.bookingUrl
      }));
    } catch (error) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        idempotencyKey.current = createSubmissionIdempotencyKey("teacher");
        draft.clearDraft();
      } else {
        setStatus("error");
